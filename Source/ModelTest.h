...
//    }
    ...
    bool test_addKeyValuePair(Iterator begin, Iterator end) {
        ...
        ECE141::Model aModel;
        aModel.openContainer("", Element::object);
        ...
        for (auto it = begin; it != end; ++it) {
            ...
            ModelNode::hashmap ahmap = aNode.getMap();
            T actual_value = std::get<T>(ahmap.at(thisKey)->get_variant());
            if(std::to_string(actual_value).substr(0, thisValue.length()).compare(thisValue) != 0) {
                return false;
            }
        }
        aModel.closeContainer("", Element::object);
        return true;
    }
